...
"""
3_Character_Generation_with_LSTM
In this module, we switch to LSTM from LSTMCell. Still it's character generator using a text dataset.
We don't use batching and every character and target is processed inside LSTM instead of looping each.
Since there's no change in the NN architecture, we don't expect any significant change in the loss results.
But, since PyTorch LSTM module process all characters at once, we may expect a slight speed up.
...
class LSTMGenerator(nn.Module):
    ...
    def forward(self, characters, hc_states=None):
        ...
        :param character: List of character indices. [time_steps]
        :param hc_states: Initial hidden&cell states pair, both [1,hidden_dim]. 
        """
        # embed layer accepts any shaped output. It just adds a new dimension holding the embedded vector.
        embedded = self.embed(characters)  # [time_steps,embedding_dim]

        # LSTM layer accepts at least 2D input (for unbatched) time_steps and embeddings.[time_steps,embedding_dim] and optional initial hidden&cell states.
        # It returns the output at each time step and final hidden&cell states pair. hidden&cell states are also at least 2D tensors (for unbatched).
        # In our unbtached case, the first dimension is number of layers and it's 1. So the hidden&cell states are [1,hidden_dim] tensors.
        # the as the output at each time step is the hidden state at that time step, output[-1] will have the same value as hc_states[0][0]
        output, hc_states = self.rnn(embedded, hc_states)   # output [time_steps,hidden_dim], hc_states ([1,hidden_dim],[1,hidden_dim])
        ...
        fc_output = self.fc(output)  # [time_steps,input_output_size]
        return fc_output, hc_states

    # [time_steps,input_output_size], [time_steps]
...
class TextGenerationExperiment(pl.LightningModule):
    ...
    def forward(self, prime_input, predict_len=100, temperature=0.25):
        ...
        output, hc_states = self.model(prime_tensor, hc_states) # output [prime_tensor_len, input_output_size], hc_states ([1,hidden_dim],[1,hidden_dim])
        ...
        for _ in range(predict_len):
            ...
            # even if t is 1d tensor, item() still works as it have only one element.
    ...
    def training_step(self, data, data_idx):
        ...
        output, hc_states = self.model(input, hc_states) # output [time_steps,input_output_size], hc_states ([1,hidden_dim],[1,hidden_dim])
