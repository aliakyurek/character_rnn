...
"""
1_Character_Generation_with_RNNCell
In this module, we implement a recurrent neural network (specifically RNNCell) based character generator using a text dataset.
RNNCell is the most basic element of sequential data processing where you iterate the input data and update the hidden state
manually.
...
class RNNGenerator(nn.Module):
    def __init__(self, input_output_size, embed_size, hidden_size):
        """
        This is the simplest RNN based generator model. 
        ...
        # this is the RNN cell that will process the input and hidden state to produce the output at each time step and update the hidden state.
        # the hidden state is not part of the layer, so we need to declare,keep it and feed it in training loop.
        self.rnn = nn.RNNCell(input_size=embed_size, hidden_size=hidden_size)
        # the output of the RNN cell is used to predict the next character, so we have a fully connected layer from hidden state to the output size.
        self.fc = nn.Linear(hidden_size, input_output_size)

    def forward(self, character, hidden_state=None):
        """
        ...
        :param hidden_state: Initial hidden state [hidden_dim]. If None, RNN will assume zeros.
        """
        ...
        # RNNCell layer accepts [embedding_dim] and optional initial hidden states
        hidden_state = self.rnn(embedded, hidden_state)  # [hidden_dim]
...
class TextGenerationExperiment(pl.LightningModule):
    ...
    def forward(self, prime_input, predict_len=100, temperature=0.25):
        ...
        for t in prime_tensor:
            # t scalar tensor
            output, hidden_state = self.model(t, hidden_state) # output [input_output_size], hidden_state [hidden_dim]
    ...
    def training_step(self, data, data_idx):
        ...
        for c in range(len(input)):
            output, hidden_state = self.model(input[c], hidden_state) # output [input_output_size], hidden_state [hidden_dim]
...
p = params['model']
model = RNNGenerator(input_output_size=len(dataset.vocab), **p)
