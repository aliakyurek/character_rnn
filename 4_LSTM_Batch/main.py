...
"""
4_Character_Generation_with_LSTM_Batch
In this module, we add batching. Still it's character generator using a text dataset.
Since there's no change in the NN architecture, we don't expect any significant change in the loss results.
But since we'll process batches, we expect a speed up in the training.
...
class LSTMGenerator(nn.Module):
    ...
    def forward(self, characters, hc_states=None):
        ...
        :param hc_states: Initial hidden and cell states pair, both [1,batch_size,hidden_dim]. 
        """
        ...
        # the first dimension is number of layers and it's 1. So the hidden and cell states are [1,batch_size,hidden_dim] tensors.
        ...
        output, hc_states = self.rnn(embedded, hc_states)  # output [batch_size,time_steps,hidden_dim] ([1,batch_size,hidden_dim],[1,batch_size,hidden_dim])
...
class TextGenerationExperiment(pl.LightningModule):
    ...
    def forward(self, prime_input, predict_len=100, temperature=0.25):
        ...
        # output [prime_tensor_len, input_output_size], hc_states ([1,hidden_dim],[1,hidden_dim])
